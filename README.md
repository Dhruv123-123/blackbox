...
1. Create an empty GitHub repository named `blackbox` and push this tree to its `main` branch.
2. The included workflow (`.github/workflows/pages.yml`) builds the app with `BASE_PATH=/blackbox/`,
   enables Pages on the repository and deploys `dist/`.
3. It is then served at `https://<your-user>.github.io/blackbox/`. Add a custom domain under
